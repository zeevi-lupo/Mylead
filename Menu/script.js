/**
 * TEAM LUPO - FULL LOGIC WITH EXACT MYLEAD STATUS MATCHER, SISA SALDO & ESTIMASI PRE-APPROVE
 */
    ...
    fetch(URL_API_POSTBACK + "?t=" + new Date().getTime())
        ...
        .then(data => {
            if(data.konversi) {
                ...
                globalPembayaranData = data.pembayaran || []; 
                renderRiwayatPembayaran(globalPembayaranData);
            } else {
                globalPostbackData = Array.isArray(data) ? data.reverse() : [];
                globalPembayaranData = [];
            }
            populateSubIdFilter(); 
            applyFilter(); 
...
// LEADERBOARD LOGIC DENGAN SISA SALDO & ESTIMASI PRE-APPROVE
...
function renderLeaderboard(data) {
    ...
    data.forEach(row => {
        ...
        if (statusLead === 'approve' || statusLead === 'pre_approve') {
            ...
            if (lolosFilter) {
                ...
                // Tambahkan penampung pendapatan_pre
                if (!kalkulasi[nama]) kalkulasi[nama] = { totalLead: 0, approve: 0, pre_approve: 0, pendapatan: 0, pendapatan_pre: 0, total_dicairkan: 0 };
                ...
                if (statusLead === 'approve') {
                    ...
                    kalkulasi[nama].pendapatan_pre += dolar; // Hitung total uang pre-approve
                }
            }
        }
    });

    // 2. Tarik Data Pembayaran untuk Mengurangi Saldo
    if (globalPembayaranData && globalPembayaranData.length > 0) {
        globalPembayaranData.forEach(payRow => {
            const statusPay = String(payRow.status || '').toLowerCase();
            if (statusPay.includes('berhasil')) {
                 const subIdPayRaw = ambilData(payRow, ['sub_id', 'sub_1', 'ml_sub1']);
                 if (subIdPayRaw !== '-') {
                     const subIdPay = subIdPayRaw.toUpperCase();
                     const nominalPay = parseFloat(ambilData(payRow, ['nominal_usd', 'nominal', 'payout'])) || 0;

                     if (kalkulasi[subIdPay]) {
                         kalkulasi[subIdPay].total_dicairkan += nominalPay;
                     } else {
                         kalkulasi[subIdPay] = { totalLead: 0, approve: 0, pre_approve: 0, pendapatan: 0, pendapatan_pre: 0, total_dicairkan: nominalPay };
                     }
                 }
            }
        });
    ...
    // 3. Susun Array dan Tentukan Sisa Saldo
    let arrayRanking = Object.keys(kalkulasi).map(kunci => {
        let memberData = kalkulasi[kunci];
        let sisaSaldo = memberData.pendapatan - memberData.total_dicairkan;
        if (sisaSaldo < 0) sisaSaldo = 0; // Cegah minus
    ...
    // Urutkan berdasarkan total pendapatan kotor tertinggi
    arrayRanking.sort((a, b) => b.pendapatan - a.pendapatan);
    
    const tbody = document.getElementById('tabel-ranking-body');
    if(!tbody) return;
    tbody.innerHTML = arrayRanking.length === 0 ? '<tr><td colspan="7" class="text-center text-muted py-4">Data kosong.</td></tr>' : '';
    ...
    arrayRanking.forEach((member, index) => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <!-- Kolom Pre-Approve dengan Nominal di Bawahnya -->
                <td style="color: var(--info); font-weight:600;">
                    ${member.pre_approve} <i class="fa-solid fa-spinner"></i>
                    <br><span style="font-size: 11px; opacity: 0.8; margin-top: 2px; display: inline-block;">($ ${member.pendapatan_pre.toFixed(2)})</span>
                </td>
                
                <td class="text-green" style="font-weight:700;">$ ${member.pendapatan.toFixed(2)}</td>
                <td style="color: #f59e0b; font-weight:800; background: rgba(245, 158, 11, 0.1); border-radius: 4px; text-align: center;">$ ${member.sisaSaldo.toFixed(2)}</td>
            </tr>`;
    });
}
